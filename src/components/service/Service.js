...
class Service extends Component {
  ...
  render() {
    ...
    const { name, imageContainer, contentClasses, paragraphs, steps, buttons } = this.props.serviceData;
    ...
    return (
      <article className={`service ${name}`} role="article"> 
        <div className={`service__imageContainer ${imageContainer.classes.join(' ')}`} >
          <LazyLoad
            debounce={false}
            offsetVertical={500}
          >
